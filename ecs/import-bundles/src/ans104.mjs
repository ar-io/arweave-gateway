...
export const processAns104 = async ({
  tx,
  filePath,
  dbRead,
  dbWrite,
  ...
  const fdStream = await fs.open(filePath);
  const stream = fdStream.createReadStream();
  const iterable = await processBundle.default(stream);
  ...
  for await (const dataItem of iterable) {
    log.info(`[data-item] ${index + 1}/${bundleLength}`);
    index += 1;
    if (dataItem.id && !alreadyPushed.has(dataItem.id)) {
      ...
      const b64EncodedTags = (dataItem.tags || []).map((tag) => ({
        name: base64url.default(tag.name),
        value: base64url.default(tag.value),
      }));

      const contentType = getTagValue(b64EncodedTags, "content-type");

      const { dataOffset, dataSize } = dataItem;

      const dataBuffer = Buffer.alloc(dataSize);
      const fd = await fs.open(filePath);
      await fd.read(dataBuffer, 0, dataSize, dataOffset);
      await fd.close();
      log.info(
        `tx/${dataItem.id} ${contentType || "application/octet-stream"}`
      );
      ...
      const dataItemDb = {
        ...
        tags: JSON.stringify(b64EncodedTags),
        quantity: 0,
      ...
      const dbTags = txTagsToRows(dataItem.id, b64EncodedTags);

      dbTags.forEach((tag) => tagsBatch.push(tag));
    } else {
      log.info(`[data-item] ans104 duplicate data-item id: ${dataItem.id}`);
    }
  }
